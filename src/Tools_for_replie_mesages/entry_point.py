...
def main():
    try:
        ...
        client_mongodb.add_data_in_entry(client_mongodb.collection_for_parser_configs, 'status_check', 'OK: Работает', 'uniq_key', client_mongodb.uniq_key)
        asyncio.get_event_loop().run_until_complete(main_task)
    finally:
        client_mongodb.add_data_in_entry(client_mongodb.collection_for_parser_configs, 'status_check', 'OK: Не работает', 'uniq_key', client_mongodb.uniq_key)